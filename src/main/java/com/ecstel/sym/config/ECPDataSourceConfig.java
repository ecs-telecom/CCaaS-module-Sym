...
@Configuration
@MapperScan(value = "com.ecstel.sym.mapper.ecp", sqlSessionFactoryRef = "BSqlSessionFactory")
public class ECPDataSourceConfig {

    private final String B_DATA_SOURCE = "BDataSource";

    // A database DataSource
    @Bean(B_DATA_SOURCE)
    @ConfigurationProperties(prefix = "spring.ecp.datasource.hikari")
    ...
    public SqlSessionFactory BSqlSessionFactory(@Qualifier(B_DATA_SOURCE) DataSource dataSource) throws Exception {
        ...
        Resource[] res = new PathMatchingResourcePatternResolver().getResources("classpath:mappers/ecp/*.xml");
